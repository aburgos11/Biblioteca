...
public class Biblioteca {
	...
	public boolean alta_libro(Libro nuevo_libro) {
		if (buscar_libro(nuevo_libro.getIsbn()) != null) {
			System.out.println("Este libro ya ha sido registrado anteriornmente");
			return false;
		}
	...
	public boolean alta_socio(Socio nuevo_socio) {
		if (buscar_socio(nuevo_socio.getDni_socio()) != null) {
			System.out.println("Ya hay un socio con los mismos datos");
			return false;
		}
	...
	public String nuevo_prestamo(int dni_socio, int isbn, Date fecha_prestamo) {
		...
		if (s.getLimite_prestamos() == 0) {
			return "El usuario ha alcanzado el límite de préstamos";
		}
	...
	public boolean devolver_libro(int isbn, Date fecha_hoy) {
	    Prestamo p = buscarPrestamo(isbn);
	...
	public static void listar_prestamos() { // Solo tengo que recorrer la lista de prestamos de esta misma clase.
		if (prestamos.isEmpty()) {
			System.out.println("No hay préstamos activos acturalmente");
			return;
		}
		for (Prestamo p : prestamos) {
